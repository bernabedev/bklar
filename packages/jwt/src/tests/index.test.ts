...
import { decode, jwt, sign, verify } from "../src"; // Adjust path if necessary
...
describe("@bklar/jwt", () => {
  describe("Helpers", () => {
    ...
    it("sign and verify a token with Uint8Array secret", async () => {
      const token = await sign(payload, TEST_SECRET_UINT8ARRAY, "HS256", {
        expiresIn: "1h",
      });
      expect(token).toBeString();

      const decodedPayload = await verify(token, TEST_SECRET_UINT8ARRAY, [
        "HS256",
      ]);
      expect(decodedPayload.userId).toBe(payload.userId);
      expect(decodedPayload.username).toBe(payload.username);
  ...
  describe("JWT Middleware", () => {
    ...
    app.post("/passthrough", passthroughHandler, {
      middlewares: [jwt({ secret, passthrough: true })],
    });
    ...
    it("should return UnauthorizedError for expired token", async () => {
      const token = await sign({ userId: "user123" }, secret, "HS256", {
        expiresIn: "0s",
      });
    ...
    it("should use custom getToken function if provided", async () => {
      ...
      const customProtectedHandler = mock(async (ctx: Context) =>
        ctx.json({ user: ctx.state.user })
      );
      customApp.post("/custom-token", customProtectedHandler, {
        middlewares: [customJwtMiddleware],
      });
    ...
    it("should throw an error if secret is not provided", () => {
      expect(() => jwt({ secret: "" } as any)).toThrow(
