...
type RoutesToClient<Routes> = {
  [Path in keyof Routes]: SplitPath<
    StripSlash<Path & string>,
    {
...
type MethodsToFns<T> = {
  [Method in keyof T]: T[Method] extends { input: infer I; output: infer O }
    ? (args: I) => Promise<O>
    : never;
};
...
type NestedClient<Routes> = UnionToIntersection<
  {
    [Path in keyof Routes]: SplitPath<
      StripSlash<Path & string>,
      { $url: string } & MethodsToFns<Routes[Path]>
    >;
  }[keyof Routes]
>;
