...
|:-------|:------------|:-------------|
| `ctx.json(data, status?, headers?)` | Returns a JSON object | `application/json` |
| `ctx.text(string, status?, headers?)` | Returns plain text | `text/plain` |
| `ctx.status(code, headers?)` | Empty response with a status code | — |
| `ctx.download(file, filename?, headers?)` | Serves a file download | auto-detected |
| `ctx.sse()` | Returns an SSE event stream | `text/event-stream` |
